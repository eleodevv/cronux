...
def mostrar_ayuda():
    ...
    # Determinar comando de desinstalación según el sistema
    uninstall_section = ""
    if platform.system() != "Windows":  # Solo mostrar en macOS/Linux
        if platform.system() == "Darwin":  # macOS
            uninstall_cmd = 'sudo /usr/local/bin/cronux/uninstall.sh'
        else:  # Linux
            uninstall_cmd = 'sudo /usr/local/cronux/uninstall.sh'
        
        uninstall_section = f"""
DESINSTALAR CRONUX-CRX:
    {uninstall_cmd}"""
...
COMANDOS:
    ...
    crx --version              Ver versión{uninstall_section}
""")
